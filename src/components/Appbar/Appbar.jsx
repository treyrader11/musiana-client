...
import { dp, closeIcon, searchIcon, hamburger, chatIcon, homeIcon, uploadVideoIcon } from "../../assets";
...
const Appbar = () => {
	...
	return (
		<header className={searchResult.posts || searchResult.users ? "header topZ" : "header"}>
			<>
			...
			<nav className="header__profile">
				{isGuest ? (
					<button className="login-btn btn" onClick={gotoLogin}>
						Login
					</button>
				) : (
					<>	
						<div className="upload icon" onClick={() => dispatch(openWizard(true))}>
